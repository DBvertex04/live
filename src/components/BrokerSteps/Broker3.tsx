// components/BrokerSteps/Broker3.tsx
'use client';
...
const Step3: React.FC<Step3Props> = ({ formData, setFormData, handleNext, handleBack }) => {
  const [selectedAreas, setSelectedAreas] = useState<string[]>(formData.areas || []);
  ...
  return (
    <>
      <h3 className="text-base font-semibold mb-2">
        Select from areas that the broker specializes in <span className="text-red-500">*</span>
      </h3>
        ...
        {selectedAreas.map((area) => (
          ...
          >
            ...
            <button onClick={() => removeTag(area)}>
              <img src="/broker/close-circle.png" alt="remove" className="w-4" />
            </button>
          </div>
        ))}
